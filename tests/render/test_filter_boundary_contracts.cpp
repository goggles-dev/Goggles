...
TEST_CASE("Filter chain wrapper boundary contract coverage", "[filter_chain][wrapper_contract]") {
    ...
    const auto filter_controls_hpp =
        std::filesystem::path(GOGGLES_SOURCE_DIR) / "src/render/chain/filter_controls.hpp";
    ...
    const auto vulkan_context_hpp =
        std::filesystem::path(GOGGLES_SOURCE_DIR) / "src/render/chain/vulkan_context.hpp";

    auto c_api_text = read_text_file(c_api_hpp);
    auto header_text = read_text_file(wrapper_hpp);
    auto filter_controls_text = read_text_file(filter_controls_hpp);
    ...
    auto vulkan_context_text = read_text_file(vulkan_context_hpp);

    REQUIRE(c_api_text.has_value());
    REQUIRE(header_text.has_value());
    REQUIRE(filter_controls_text.has_value());
    REQUIRE(canonical_filter_controls_text.has_value());
    REQUIRE(canonical_vulkan_context_text.has_value());
    REQUIRE(vulkan_context_text.has_value());
    ...
    REQUIRE(filter_controls_text->find("#include <goggles/filter_chain/filter_controls.hpp>") !=
            std::string::npos);
    REQUIRE(vulkan_context_text->find("#include <goggles/filter_chain/vulkan_context.hpp>") !=
            std::string::npos);
