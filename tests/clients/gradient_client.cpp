...
int main() {
    ...
    xdg_toplevel_set_title(toplevel.get(), "gradient_client");
    ...
    for (int y = 0; y < shm_buffer->height; ++y) {
        for (int x = 0; x < shm_buffer->width; ++x) {
            const int value = (x * 255) / (shm_buffer->width - 1);
            const uint32_t pixel = goggles::test::pack_argb8888(static_cast<uint8_t>(value),
