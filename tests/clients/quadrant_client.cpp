...
int main() {
    ...
    xdg_toplevel_set_title(toplevel.get(), "quadrant_client");
    ...
    const uint32_t green = goggles::test::pack_argb8888(0, 255, 0, 255);
    const uint32_t blue = goggles::test::pack_argb8888(0, 0, 255, 255);
    const uint32_t white = goggles::test::pack_argb8888(255, 255, 255, 255);

    for (int y = 0; y < shm_buffer->height; ++y) {
        for (int x = 0; x < shm_buffer->width; ++x) {
            const bool right = x >= (shm_buffer->width / 2);
            const bool bottom = y >= (shm_buffer->height / 2);
            ...
            shm_buffer
                ->data[(static_cast<std::size_t>(y) * static_cast<std::size_t>(shm_buffer->width)) +
